...
Discovery.hikvisionProbe = function(options, callback) {
	...
	var cams = {}
		, errors = []
		, messageID = guid()
		, request = Buffer.from('<?xml version="1.0" encoding="utf-8"?><Probe><Uuid>' + messageID + '</Uuid><Types>inquiry</Types></Probe>')
	...
	const listener = function(msg, rinfo) {
		...
		parseString(msg.toString(), function(err, result) {
			if (!cams[result.ProbeMatch.DeviceSN[0]]) {
				cams[result.ProbeMatch.DeviceSN[0]] = result.ProbeMatch;
				Discovery.emit('hikvisionDevice', result, rinfo);
			}
		});
	};
	...
	socket.send(request, 0, request.length, 37020, '239.255.255.250');
	...
	const listener = function(msg, rinfo) {
		parseSOAPString(msg.toString(), function(err, data, xml) {
			...
			if (err || !data[0].probeMatches) {
				...
				if (data.probeMatches.probeMatch) {
